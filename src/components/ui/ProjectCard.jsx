    ...
    <div className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300">
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 flex items-center gap-3  ">
          {demoUrl && (
            <a
              ...
              className="p-2.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-primary/30 hover:bg-primary/50 hover:scale-110 transition-all duration-300 "
              title="View Demo"
            >
        ...
        <div className="absolute top-4 left-4">
          <span className="px-3 py-1 text-xs font-medium text-white bg-black/40 backdrop-blur-xl border-border-white/20 rounded-full">{category}</span>
        </div>
      </div>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {technologies && technologies.map((tech, index) => (
            <span key={index} className="px-3 py-1 text-xs text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-colors duration-200 hover:cursor-pointer">
              {tech}
            </span>
          ))}
        </div>
